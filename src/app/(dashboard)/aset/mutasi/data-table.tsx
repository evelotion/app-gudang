...
export default function DataTableMutasi({ data, onEdit, onRefresh }: { data: any[], onEdit: (item: any) => void, onRefresh: () => void }) {
  ...
  // LOGIKA GROUPING BERDASARKAN PILIHAN DROPDOWN
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="text-sm border border-slate-200 rounded-md py-1.5 px-3 outline-none focus:ring-2 focus:ring-indigo-500/20 bg-white shadow-sm"
            >
              <option value="golonganAset">Golongan Aset</option>
              <option value="lokasiAwal">Lokasi Awal</option>
              <option value="lokasiTujuan">Lokasi Tujuan</option>
            </select>
          </div>
                ...
                <React.Fragment key={groupName}>
                  ...
                  <TableRow className="bg-slate-100/80 hover:bg-slate-100 border-t-2 border-slate-200">
                    <TableCell colSpan={13} className="font-bold text-indigo-900 py-3 uppercase text-xs tracking-wider">
                      {groupLabels[groupBy]}: {groupName}
                    </TableCell>
                  </TableRow>

                  {/* BARIS DATA PER GROUP */}
                  {items.map((row: any) => {
                  ...
                  <TableRow className="bg-indigo-50/50 border-b-2 border-indigo-100 hover:bg-indigo-50/50">
                    <TableCell colSpan={5} className="text-right font-bold text-indigo-900 py-3">
                      SUBTOTAL {String(groupName).toUpperCase()}:
                    </TableCell>
