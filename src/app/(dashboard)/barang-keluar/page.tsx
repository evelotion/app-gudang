...
import { Plus, Trash2, AlertTriangle, Search, PackageOpen, Truck, Box, CheckCircle2, RefreshCw } from "lucide-react";
import { toast } from "sonner";
import { PageHeader } from "@/components/PageHeader";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";

// ... (Interface BarangTipe, CartItem, FormDataReq TETAP SAMA) ...
interface BarangTipe {
  id: string; kode_barang: string; nama_barang: string; satuan: string; stok: number; stok_min: number; nomorator: string;
}
interface CartItem {
  barangId: string; kode_barang: string; nama_barang: string; satuan: string; qty: number; nomorator: string;
}
interface FormDataReq {
  media_request: string; no_dokumen: string; tanggal_dokumen: string; cabang: string; tanggal_request: string; jenis_permintaan: string; pic_nama: string; items: CartItem[];
}
...
export default function BarangKeluar() {
  ...
  useEffect(() => {
    ...
    getSession().then((session) => { if (session && session.nama) setPicNama(session.nama); else setPicNama("User Tidak Dikenal"); });
  ...
  const handlePilihBarang = (barang: BarangTipe) => {
    setSelectedBarang(barang); setSearchQuery(`${barang.kode_barang} - ${barang.nama_barang}`); setInputNomorator(barang.nomorator || ""); setShowDropdown(false);
  };
  ...
  const handleAddItem = () => {
    ...
    setItems([...items, { barangId: selectedBarang.id, kode_barang: selectedBarang.kode_barang, nama_barang: selectedBarang.nama_barang, satuan: selectedBarang.satuan, qty: inputQty, nomorator: inputNomorator }]);
  ...
  const handlePreSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setFormDataCache({ media_request: formData.get("media_request") as string, no_dokumen: formData.get("no_dokumen") as string, tanggal_dokumen: formData.get("tanggal_dokumen") as string, cabang: formData.get("cabang") as string, tanggal_request: formData.get("tanggal_request") as string, jenis_permintaan: formData.get("jenis_permintaan") as string, pic_nama: picNama, items: items });
    setShowConfirm(true);
  };
  ...
  return (
    <main className="flex-1 p-6 lg:p-10 w-full max-w-[1600px] mx-auto space-y-8 pb-10">
      ...
      <PageHeader 
        title="Barang Keluar" 
        description="Kelola form pengeluaran barang dan pantau status fisik pengiriman logistik ke Cabang."
      />

      {/* --- MENU TABS --- */}
      <div className="flex border-b border-slate-200">
        <button onClick={() => setActiveTab("form")} className={`flex items-center gap-2 px-6 py-3.5 font-bold transition-all border-b-2 ${activeTab === "form" ? "border-indigo-600 text-indigo-700 bg-indigo-50/50" : "border-transparent text-slate-500 hover:text-slate-800 hover:bg-slate-50"}`}>
          <Plus className="w-4 h-4" /> Form Input Pengeluaran
        </button>
        <button onClick={() => setActiveTab("monitoring")} className={`flex items-center gap-2 px-6 py-3.5 font-bold transition-all border-b-2 ${activeTab === "monitoring" ? "border-indigo-600 text-indigo-700 bg-indigo-50/50" : "border-transparent text-slate-500 hover:text-slate-800 hover:bg-slate-50"}`}>
      ...
      {activeTab === "monitoring" && (
        <Card className="animate-in fade-in duration-300">
          <div className="p-6 border-b border-slate-100 flex flex-wrap gap-4 justify-between items-center bg-slate-50/50 rounded-t-2xl">
            <div>
                ...
                {reqData.length > 0 ? reqData.map((req) => (
                  <tr key={req.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-center">
                      {req.status === "PACKING" ? (
                        <button onClick={() => handleUpdateStatus(req.id, req.no_dokumen)} className="px-4 py-2 rounded-lg bg-indigo-50 hover:bg-indigo-600 text-indigo-700 hover:text-white font-bold text-xs transition-colors border border-indigo-100 inline-flex items-center gap-2">
