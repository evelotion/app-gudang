...
import { X, Loader2, Sparkles, CheckCircle2, ListEnd } from "lucide-react";
import { updateRegistrasiAset, createBulkRegistrasiAset } from "@/actions/aset";
...
export default function FormRegistrasi({ initialData, onSuccess, onCancel }: { initialData?: any, onSuccess: () => void, onCancel: () => void }) {
  ...
  // Pantau semua perubahan form secara real-time
  const formValues = watch();

  // Auto-Detect Golongan
  useEffect(() => {
    if (formValues.nomorRegisterAset !== undefined) {
      const lines = formValues.nomorRegisterAset.split('\n');
  ...
  }, [formValues.nomorRegisterAset, setValue, getValues]);

  // Real-time Generate Preview
  useEffect(() => {
    const splitLines = (str: string) => str ? str.trim().split('\n').map(s => s.trim()) : [];
    
    const noReg = splitLines(formValues.nomorRegisterAset || "");
    const nama = splitLines(formValues.namaAset || "");
    const gol = splitLines(formValues.golonganAset || "");
    const jml = splitLines(formValues.jumlah || "");
    const tgl = splitLines(formValues.tanggalPerolehan || "");
    const hrg = splitLines(formValues.hargaPerolehan || "");
    const cabang = splitLines(formValues.cabangUnitKerja || "");
    const user = splitLines(formValues.userPengguna || "");
    const lokasi = splitLines(formValues.lokasiPosisiAset || "");
    ...
    for (let i = 0; i < maxRows; i++) {
      // Hanya tampilkan jika setidaknya ada nama atau nomor register yang diisi (hindari blank row)
      if (!noReg[i] && !nama[i]) continue;
      ...
      rows.push({
        ...
        hargaPerolehan: Number(String(hrg[i] || hrg[0] || "0").replace(/\D/g, '')),
  ...
  const onSubmit = async (data: FormValues) => {
    if (previewRows.length === 0) {
      toast.error("Form masih kosong!");
      return;
    }

    setIsSubmitting(true);
    const toastId = toast.loading(`Menyimpan ${previewRows.length} data...`);
    ...
    try {
      ...
      if (initialData) {
        const payload = {
          ...
          namaAset: nama[0] || "-",
          golonganAset: gol[0] || "-",
          jumlah: Number(String(jml[0] || 1).replace(/\D/g, '')),
          tanggalPerolehan: parseDateFlexible(tgl[0]), 
          hargaPerolehan: Number(String(hrg[0] || 0).replace(/\D/g, '')),
          cabangUnitKerja: cabang[0] || "-",
          userPengguna: user[0] || "-",
          lokasiPosisiAset: lokasi[0] || "-",
          inputerName: data.inputerName,
          supervisorName: data.supervisorName || ""
        };
        res = await updateRegistrasiAset(initialData.id, payload as any);
      } else {
        const payloads = [];
        for (let i = 0; i < maxRows; i++) {
          if (!noReg[i]) continue;
          ...
          payloads.push({
            tanggalInput: new Date(data.tanggalInput), 
            nomorRegisterAset: noReg[i],
            namaAset: nama[i] || nama[0] || "-",
            golonganAset: gol[i] || gol[0] || "-",
            jumlah: Number(String(jml[i] || jml[0] || 1).replace(/\D/g, '')),
            tanggalPerolehan: parseDateFlexible(tgl[i] || tgl[0]), 
            hargaPerolehan: Number(String(hrg[i] || hrg[0] || 0).replace(/\D/g, '')),
            cabangUnitKerja: cabang[i] || cabang[0] || "-",
            userPengguna: user[i] || user[0] || "-",
            lokasiPosisiAset: lokasi[i] || lokasi[0] || "-",
        ...
        res = await createBulkRegistrasiAset(payloads);
      }
      
      if (!res?.success) {
        toast.error(res?.message || "Gagal menyimpan data ke database (Cek duplikasi Nomor Register)", { id: toastId });
        setIsSubmitting(false);
        return;
      }

      toast.success(`Berhasil! ${previewRows.length} data tersimpan.`, { id: toastId });
      onSuccess();
    } catch (error) {
      toast.error("Gagal menyimpan data karena kesalahan sistem.", { id: toastId });
      console.error(error);
    } finally {
      setIsSubmitting(false);
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-slate-900/60 backdrop-blur-sm overflow-y-auto">
      <div className="bg-white w-full max-w-6xl rounded-2xl shadow-2xl border border-slate-200 flex flex-col max-h-[90vh] my-auto">
        ...
        <div className="flex justify-between items-center p-5 border-b border-slate-100 bg-slate-50/50 rounded-t-2xl">
          <div>
            <h2 className="text-xl font-bold text-slate-800">
              {initialData ? "Edit Registrasi Aset" : "Bulk Insert Aset"}
            </h2>
            <p className="text-xs text-slate-500 mt-0.5">
              Copy-paste langsung dari file Excel. Tabel preview otomatis muncul di bawah.
            </p>
          </div>
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar flex-grow">
          ...
          <form id="asetForm" onSubmit={handleSubmit(onSubmit)} className="space-y-5 mb-8"> 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              <div className="space-y-1">
                ...
                <textarea {...register("nomorRegisterAset")} rows={3} className="w-full text-sm p-2.5 border border-slate-300 rounded-lg outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500" placeholder="Paste kolom ini dari Excel..." />
                {errors.nomorRegisterAset && <p className="text-[10px] text-rose-500">{errors.nomorRegisterAset.message}</p>}
              </div>
              
              <div className="space-y-1">
                <label className="text-xs font-semibold text-slate-700">Nama Aset</label>
                <textarea {...register("namaAset")} rows={3} className="w-full text-sm p-2.5 border border-slate-300 rounded-lg outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500" placeholder="Paste kolom ini dari Excel..." />
              </div>
              ...
              <div className="space-y-1">
                ...
                <textarea {...register("golonganAset")} rows={3} className="w-full text-sm p-2.5 border border-slate-300 rounded-lg outline-none bg-amber-50/30" readOnly placeholder="Otomatis terisi..." />
              </div>
              ...
              <div className="space-y-1">
                ...
                <textarea {...register("tanggalPerolehan")} rows={3} className={`w-full text-sm p-2.5 border rounded-lg ${errors.tanggalPerolehan ? "border-rose-500" : "border-slate-300"}`} placeholder="DD/MM/YYYY" />
              </div>
          ...
          {/* TAMPILAN 2: TABLE PREVIEW LANGSUNG DI BAWAH FORM */}
          {previewRows.length > 0 && (
            <div className="space-y-3 animate-in fade-in duration-300 pt-6 border-t border-slate-100">
              <div className="flex items-center gap-2 text-slate-800">
                <ListEnd className="w-5 h-5 text-indigo-600" />
                <h3 className="text-lg font-bold">Preview Data Real-time</h3>
                <span className="bg-indigo-100 text-indigo-700 px-2.5 py-0.5 rounded-md text-xs font-bold">{previewRows.length} Baris</span>
              </div>
        ...
        <div className="flex justify-end gap-3 p-5 border-t border-slate-100 bg-slate-50/50 rounded-b-2xl">
          ...
          <button form="asetForm" type="submit" disabled={isSubmitting || previewRows.length === 0} className="flex items-center gap-2 px-6 py-2.5 text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg shadow-lg disabled:opacity-70 disabled:cursor-not-allowed transition-all">
            {isSubmitting ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle2 className="w-4 h-4" />}
            Simpan {previewRows.length > 0 ? previewRows.length : ''} Data
          </button>
        </div>
      </div>
    </div>
